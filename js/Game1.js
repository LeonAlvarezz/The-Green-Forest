...
var lives = 3
var e
const respawnPoint = { x: 100, y: 810 };
var isGameOver = false
var wKey, aKey, sKey, dKey, pKey, gKey


class UIScene extends Phaser.Scene {
    constructor() {
        super({ key: 'UIScene' });
        e = new Phaser.Events.EventEmitter();
    ...
    create() {
        ...
        infoText.setShadow(5, 5, '#000000', 5);
...
class GameState extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.audio('soundTrack', 'sounds/background_music.mp3');
        this.load.audio('jumpSound', 'sounds/cartoon-jump.mp3');
    ...
    create() {
        ...
        const jumpSound = this.sound.add('deathSound');
    ...
    update() {
        if (player.y > 1000) {
            lives -= 1;
            const deathSound = this.sound.add('deathSound');
            deathSound.play()
            console.log("You have " + lives + " live left")
        ...
        if (lives < 0) {
            ...
            if (player.x < 0) {
                player.x = 0; // Prevent moving left
            } else if (player.x > this.game.config.width) {
                player.x = this.game.config.width; // Prevent moving right
            }
            ...
            if (spaceKey.isDown || wKey.isDown) {
                if (isTouchingGround) {
    ...
    handleKeyHold() {
        ...
        delay: 1000
        this.scene.stop("UIScene")
        this.scene.start('WinScene', { score: seedStored, lives: lives });
...
var config = {
    ...
    scene: [TitleScreen, GameState, GameOverScene, WinScene, AboutScene, UIScene], // Include both UI scene and Game scene
